...
# 단 암호는 특수 문자없이 숫자와 소문자 알파벳으로 구성된 6자리 문자로 되어 있다.(mars06)
...
def split_first_characters():
    ...
    return [
        characters[:group_size],                      # 0 ~ 11
        characters[group_size:group_size*2],          # 12 ~ 23
        characters[group_size*2:]                     # 24 ~ 끝
    ]
...
if __name__ == '__main__':
    ...
    # 브루트포스 공격 설정
    first_chars = split_first_characters()
    # 소문자, 숫자
    rest_chars = string.ascii_lowercase + string.digits  
    #길이가 6자리인 모든 문자, 숫자의 조합 완성
    combinations = itertools.product(
        first_chars,
        rest_chars, rest_chars, rest_chars, rest_chars, rest_chars
    )
    #필요한 인수를 튜플로 조합함
    ...
    for first_char in first_chars:
        p = Process(target=unlock_zip, args=(
            zip_bytes, first_char, found_flag, start_time, time_str
        ))
        p.start()
        processes.append(p)
